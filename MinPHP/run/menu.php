...
?>
    ...
    <div class="list">
        <ul class="list-unstyled">
            ...
            foreach($list as $v) {
                if (session('issupper') || in_array($v['aid'], $show)) {
            ?>
    ...
    <form action="?act=cate" method="post">
        ...
        <div style="float:right;margin-right:20px;">
            <button class="btn btn-success btn-sm" name="op" value="add">新建分类</button>
        </div>
        <?php } ?>
    </form>
<?php } else{
    ...
    $list = select($sql);?>
    <div class="form-group">
        <input type="text" class="form-control" id="searchapi" placeholder="尝试搜索您想要的接口" onkeyup="search('api',this)">
    </div>
    <div class="list">
    ...
    <form action="?act=api&tag=<?php echo $_GET['tag'];?>&op=add" method="post">
        <?php if(is_supper()){?>
            ...
            <div style="float:right;margin-right:20px;">
                ...
                <button class="btn btn-success btn-sm">新建接口</button>
            </div>
        <?php } ?>
    </form>
<?php } ?>
...
<script>
    ...
    window.onload=function(){
        ...
        var navbarButton = '<div onclick="navbar(this)" ' +
            'style="text-align:center;line-height:120px;border-bottom-right-radius:5px;cursor:'+cursor+';border-top-right-radius:5px;width:12px;height:120px;background: rgba(91,192,222, 0.8);position:fixed;left:0;top:50%;color:#fff;box-shadow: 0px 0px 0px 1px #cccccc;">' +
            status_flg +
            '</div>'
